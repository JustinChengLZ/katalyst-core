...
func (s *DeviceAffinityStrategy) Bind(
	...
	if len(allocatedDevices) == devicesToAllocate {
		// Ensure the final allocation satisfies perfect device affinity at the lowest priority level,
		...
		if requiredDeviceAffinity && len(affinityGroupsByPriority) > 0 {
			...
			if err = s.validatePerfectDeviceAffinityAtLowestPriority(
...
// validatePerfectDeviceAffinityAtLowestPriority checks that lowest-priority
// allocations still satisfy the "perfect" device affinity requirement, i.e.
// we only touch the minimum number of groups implied by the request size.
func (s *DeviceAffinityStrategy) validatePerfectDeviceAffinityAtLowestPriority(
	...
	if allocatedGroups != expectedGroups {
		return fmt.Errorf(
			"required device affinity cannot be satisfied at lowest priority: request=%d, maxGroupSize=%d, expectedGroups=%d, allocatedGroups=%d",
