...
func TestBind(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			sortedDevices: []string{"gpu-4", "gpu-2"},
			expectedResult: &allocate.AllocationResult{
		...
		},
		{
			...
			ctx: &allocate.AllocationContext{
				...
				// 1st level: [gpu-1, gpu-2], [gpu-3, gpu-4], [gpu-5, gpu-6], [gpu-7, gpu-8]
				// 2nd level: [gpu-1, gpu-2, gpu-3, gpu-4], [gpu-5, gpu-6, gpu-7, gpu-8]
				DeviceTopology: &machine.DeviceTopology{
					Devices: map[string]machine.DeviceInfo{
						"gpu-1": {
							DeviceAffinity: map[machine.AffinityPriority]machine.DeviceIDs{
		...
		},
		{
			...
			sortedDevices: []string{"gpu-1", "gpu-5", "gpu-2", "gpu-6", "gpu-7", "gpu-8"},
			expectedResult: &allocate.AllocationResult{
		...
		},
		{
			...
			ctx: &allocate.AllocationContext{
				...
				DeviceReq: &pluginapi.DeviceRequest{
					DeviceName:      "gpu",
					ReusableDevices: []string{"gpu-1", "gpu-5", "gpu-9"},
					DeviceRequest:   8,
				},
				DeviceTopology: &machine.DeviceTopology{
					Devices: map[string]machine.DeviceInfo{
			...
			// Allocate gpu-1, gpu-5 and gpu-9 first because they are reusable devices
